...
from ..widgets.module_widgets import HistWidget
...
class Histogram(HardwareModule, AcquisitionModule):
    addr_base = 0x40100000
    name = 'histogram'
    _widget_class = HistWidget
    # run = ModuleProperty(ScopeAcquisitionManager)
    _gui_attributes = ["input1",
                       "input2",
                       "duration",
    ...
    def _bins_ch(self, ch):
        if ch not in [1, 2]:
            raise ValueError("channel should be 1 or 2, got " + str(ch))
        return self._bins_ch1 if ch == 1 else self._bins_ch2
    ...
    @property
    def data_x(self):
        # return np.array((self._bins_ch(1), self._bins_ch(2)))
        return self._bins_ch(1)
    ...
    def _new_run_future(self):
        ...
        if self._is_rolling_mode_active() and self.running_state == \
            ...
            super(Histogram, self)._new_run_future()
