    ...
    def set_value(self, instance, waveform):
        ...
        if not waveform in instance.waveforms:
            ...
            if waveform == 'gaussiannoise':
                ...
                instance._waveform = 'gaussiannoise'
                ...
                return waveform
            elif waveform == 'uniformnoise':
                # current amplitude becomes rms amplitude
                rmsamplitude = instance.amplitude
                # set alrady to noise to switch behavior of amplitude
                instance._waveform = 'uniformnoise'
                # amplitude now deals with the settings
                # (fpga amplitude to max, saving of rmsamplitude,
                # filling of correctly scaled data)
                instance.amplitude = rmsamplitude
                ...
                # can return already now
                return waveform
            elif waveform == 'amplitudenoise':
                # current amplitude becomes rms amplitude
                rmsamplitude = instance.amplitude
                # set alrady to noise to switch behavior of amplitude
                instance._waveform = 'amplitudenoise'
                # amplitude now deals with the settings
                # (fpga amplitude to max, saving of rmsamplitude,
                # filling of correctly scaled data)
                instance.amplitude = rmsamplitude
...
class AsgAmplitudeAttribute(FloatRegister):
    ...
    def get_value(self, obj):
        if obj.waveform == 'gaussiannoise':
            return obj._rmsamplitude
        elif obj.waveform == 'uniformnoise':
            return obj._rmsamplitude
        elif obj.waveform == 'amplitudenoise':
            return obj._rmsamplitude
        else:
    ...
    def set_value(self, obj, val):
        if obj.waveform == 'gaussiannoise':
            ...
            obj.data = obj._gaussian_noise_distribution()
            ...
            obj._rmsamplitude = val
            # fill normal-distributed data into data memory
            obj.data = obj._uniform_noise_distribution()
            # multiplier set to 1.0 to benefit from full available resolution
            super(AsgAmplitudeAttribute, self).set_value(obj, 1.0)
        elif obj.waveform == 'amplitudenoise':
            # internally memorize the amplitude as it is not directly
            ...
            obj._rmsamplitude = val
            # fill normal-distributed data into data memory
            obj.data = obj._amplitude_noise_distribution()
            # multiplier set to 1.0 to benefit from full available resolution
            super(AsgAmplitudeAttribute, self).set_value(obj, 1.0)
        else:
...
def make_asg(channel=0):
    ...
    class Asg(HardwareModule, SignalModule):
        ...
        def _gaussian_noise_distribution(self):
            """
        ...
        def _uniform_noise_distribution(self):
            """
            returns an array of data_length samples of a uniform
            distribution with rms=self._rmsamplitude
            """
            if self._rmsamplitude == 0:
                return np.zeros(self.data_length)
            else:
                return np.random.uniform(-self._rmsamplitude,self._rmsamplitude,
                                        size=self.data_length)
            
        def _amplitude_noise_distribution(self):
            """
            returns an array of data_length samples of a uniform
            distribution with rms=self._rmsamplitude
            """
            if self._rmsamplitude == 0:
                ...
                Z=np.random.uniform(0,1,size=self.data_length)
                # k=0.5
                # U0=1
                # N=1
                alpha=5
                att=20
                return (self.offset + self.frequency*np.arcsin((1/alpha)*np.sqrt(2*self.amplitude*np.log(1+(np.exp(alpha**2/2/self.amplitude)-1)*Z))))/att
        ...
        waveforms = ['sin', 'cos', 'ramp', 'halframp', 'square', 'dc',
                     'gaussiannoise','uniformnoise','amplitudenoise']
